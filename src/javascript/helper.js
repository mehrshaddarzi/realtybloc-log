 ...
 */
rbl_js.line_chart = function (tag_id, title, label, data) {

    // Get Element By ID
    let ctx = document.getElementById(tag_id).getContext('2d');

    // Check is RTL Mode
    if (rbl_js.is_active('rtl')) {
 ...
 * Create pie Chart JS
 */
rbl_js.pie_chart = function (tag_id, label, data, label_callback = false) {
        ...
        plugins: [{
            afterDraw: function (chart) {
                if (chart.data.datasets[0].data.every(x => x == 0) === true) {
                    ...
                    ctx.fillText(rbl_js._('no_data'), width / 2, height / 2);
                    ctx.restore();
                }
            }
        }]
 ...
 * @param meta_box
 */
rbl_js.chart_id = function (meta_box) {
 ...
 */
rbl_js.random_color = function (i = false) {
    let colors = [
        [243, 156, 18, "#f39c12"],
        [52, 152, 219, "#3498db"],
        [192, 57, 43, "#c0392b"],
 ...
 * Show Domain Icon
 */
rbl_js.site_icon = function (domain) {
 ...
 * @param type
 */
rbl_js.wordpress_postbox_ajax = function (type = 'enable') {
 ...
 * @returns {*}
 */
rbl_js.number_format = function (number, decimals, dec_point, thousands_point) {
 ...
 * @param Dom_2
 */
rbl_js.set_equal_height = function (Dom_1, Dom_2) {
 ...
 * @returns {string}
 */
rbl_js.Create_Half_PostBox = function (div_class, div_id) {
