...
class Template {
	...
	function shortlink_locate_template( $template_name, $template_path = '', $default_path = '' ) {
		...
		if ( ! $default_path ) :
			$default_path = \WP_REVIEWS_INSURANCE::$plugin_path . 'template/'; // Path to the template folder
		endif;
	...
	function shortlink_get_template( $template_name, $args = array(), $tempate_path = '', $default_path = '' ) {
		...
		if ( ! file_exists( $template_file ) ) :
			_doing_it_wrong( __FUNCTION__, __("Template Shortlink Not Found", 'realty-bloc-log') , '1.0.0' );
			return;
		endif;
